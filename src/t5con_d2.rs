...
//#[cfg(test)]
use crate::w1types::{HeadByteStruct, PackAddedStatus, PackType, Ttl, WSQueueState, WTypeErr};
    ...
    >
{
        ...
        F: FnMut(&[u8], PackType, bool) -> Result<f32, String>,
    {
        ...
        let topol = cp
            .sheme()
            .get_topol(tbyte.unwrap_or(0), crate::w1types::PackTypeGroup::Fback);

        #[cfg(test)]
        {
            assert!(topol.is_none() && cp.sheme().is_one());
            assert!(self.non_alloc_buf.is_empty());
        }
        ...
        let real_payload_len = self.fback_queue.payload_len_in_bytes();

        let (payload_len_final, _is_fake) = self
            .get_pack_len_and_is_fake(&cp, force_no_edit_pack_type, false, &real_payload_len, &ovh)
            .map_err(WTypeErr::WorkTimeErr)?;

        #[cfg(test)]
        {
            // assert!(payload_len_final >= self.non_alloc_buf);
        }
    ...
    fn get_tb(&mut self) -> Option<u8> {
        if let Some(x) = &self.tricky_byter {
            ...
            #[cfg(test)]
            {
                assert!(self.tricky_byter.is_none())
            }
            None
        }
    }
