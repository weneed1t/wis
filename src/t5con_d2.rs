    ...
    >
{
    ...
    fn send_any_pack<F>(
        ...
        _current_time: f32,
        _what_should_send: WhatSend,
        force_no_edit_pack_type: Option<bool>,
    ) -> Result<(), WTypeErr>
    where
        F: FnMut(&[u8], PackType, bool) -> Result<f32, String>,
    {
        self.non_alloc_buf.clear();
        ...
        let (group_pack_type, can_trim, real_payload_len) = match &_what_should_send {
        ...
        debug_assert!(self.non_alloc_buf.is_empty());
        ...
        let (payload_len_final, _is_fake) = self
        ...
        //  debug_assert!(payload_len_final >= self.non_alloc_buf);

        let antoher = (ttl_for_init, &id_s, tbyte, topol);

        self.is_fback(&mtu, &ovh, &payload_len_final, &real_payload_len, antoher)?;
    ...
    fn is_fback(
        ...
        real_payload_len: &usize,

        antoher: (Option<(&Ttl, bool)>, &Identified, Option<u8>, &PackTopology),
    ) -> Result<(), WTypeErr> {
        self.paste_fback_data_in_non_alloc(ovh, real_payload_len)?;

        let end_range = {
            self.non_alloc_buf
                .get_mut(
                    *ovh..ovh
                        .checked_add(*payload_len_final)
                        .ok_or(WTypeErr::WorkTimeErr(
                            "payload_len_final +ovh overflow".to_string(),
                        ))?,
                )
                .ok_or(WTypeErr::WorkTimeErr(
                    "non_alloc_buf out of range".to_string(),
                ))?
        };
        ...
        init_all_pack_to_send(
            end_range,
            antoher.3,
            HeadByteStruct::new(), // потом измененить
            &self.ctrs.my_fback,
            PackType::Fback,
            antoher.1,
            antoher.2,
    ...
    fn paste_fback_data_in_non_alloc(
        &mut self,
        ovh: &usize,
        real_payload_len: &usize,
    ) -> Result<(), WTypeErr> {
        let real_range = self
            ...
            .get_mut(
                *ovh..ovh
                    .checked_add(*real_payload_len)
                    .ok_or(WTypeErr::WorkTimeErr(
                        "real_payload_len +ovh overflow".to_string(),
                    ))?,
            )
            .ok_or(WTypeErr::WorkTimeErr(
                "non_alloc_buf out of range".to_string(),
        ...
        self.fback_queue
            .copy_ctrs_pack_to_slice(real_range, false)
            .map_err(WTypeErr::WorkTimeErr)?;
